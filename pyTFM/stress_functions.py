...
def normal_vector(coords,dims=None): # works only with intervall 2 confirmed!,
    ...
    if dims:
        ...
        n_array[coords[:,0], coords[:,1], :] = n  # filling array (note: first axis is y second is x--> coords still and as yx arrangement
        return n,n_array
...
def normal_vector_from_graph(graph,points,dims=None): ## implement larger window (rather difficult)  ##
    ...
    for key,values in graph.items():
        if len(values)==2:
            ...
            n_vec[[0,1]]=n_vec[[1,0]] # from https://stackoverflow.com/questions/1243614/how-do-i-calculate-the-normal-vector-of-a-line-segment/1243676#1243676
            n_vec[1]=-n_vec[1]
...
def coefficient_of_variation(mask,x,border_pad):
    ...
    return np.nanstd(x[mask_cp]) / np.abs(np.nanmean(x[mask_cp])) # absoulte value of mean is an alternative deffinition
...
def all_stress_measures(S_nodes,nodes,dims):
    ...
    if not dims:
        sig_x = np.zeros((int(np.sqrt(len(S_nodes))), int(np.sqrt(len(S_nodes))), 2, 2))
        sig_y = np.zeros((int(np.sqrt(len(S_nodes))), int(np.sqrt(len(S_nodes))), 2, 2))
        tau_xy = np.zeros((int(np.sqrt(len(S_nodes))), int(np.sqrt(len(S_nodes))), 2, 2))
    else:
        sig_x = np.zeros(dims)
        sig_y = np.zeros(dims)
        tau_xy = np.zeros(dims)
    sig_x[nodes[:, 2], nodes[:, 1]] = S_nodes[:, 0]  # sigma_x
    sig_y[nodes[:, 2], nodes[:, 1]] = S_nodes[:, 1]  # sigma_y
    tau_xy[nodes[:, 2], nodes[:, 1]] = S_nodes[:, 2]  # sigma_yx
    ...
    return  sigma_max,sigma_min,tau_max, phi_n,phi_shear,sigma_avg
...
def evaluate_all_stress_measures(lines_interpol, borders,norm_levels=["points","lines","cells"],types=["t_vecs","tn","ts"],show_histogramm=False):
    results={}
    for vtype,n_level in product(types,norm_levels):
        results[(vtype,n_level)] = mean_stress_vector_norm(lines_interpol, borders,exclude_colony_edge=True, norm_level=n_level,vtype=vtype)
