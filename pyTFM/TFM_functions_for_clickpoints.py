# function integrating Traction force microscopy into a clickpoints database
...
class cells_masks():
        ...
        for frame in tqdm(frames):
            ...
            for obj_id, r in enumerate(regions):  # iterating through all objects
                for mask_name, index in self.indices.items():  # iterating through all max types
                    ...
                    # writing to dict only the coordinates of true values
                    self._masks_dict[frame][obj_id][mask_name] = (coords_final, shape)
    ...
    def get_com_frame(self, frame):
        ...
        for cell_id in self.masks_dict[frame].keys():
            com = self.masks_dict[frame][cell_id]["com"]
    ...
    def reconstruct_mask(self, frame, cell_id, mtype, raise_error=True, fill_holes=False, cut_close_to_edge=True):
        ...
        mask = None
        ind_shape = self.masks_dict[frame][cell_id][mtype]
    ...
    def reconstruct_masks_frame(self, frame, mtype="mask", raise_error=True, fill_holes=False, obj_ids=[],
                                indices=None):
        # retrieves the list of all masks and cell ids in one frame
        ret = []
...
def try_to_load_deformation(path, frame, warn=False):
    """
    loading the deformations from a given frame. If deformations are not found either raises an error
    ...
    """
    try:
        if default_parameters["output type"] == "npy":
            u = np.load(os.path.join(path, frame + "u.npy"))
            v = np.load(os.path.join(path, frame + "v.npy"))
        if default_parameters["output type"] == "txt":
            u = np.loadtxt(os.path.join(path, frame + "u.txt"))
            v = np.loadtxt(os.path.join(path, frame + "v.txt"))
...
def try_to_load_traction(path, frame, warn=False):
    ...
    try:
        ...
        if default_parameters["output type"] == "txt":
            t_x = np.loadtxt(os.path.join(path, frame + "tx.txt"))
            t_y = np.loadtxt(os.path.join(path, frame + "ty.txt"))
...
def get_db_info_for_analysis(db):
    ...
    # TODO: think about optimizing that// this essentially loads one image per frame each time it is called
...
def general_properties(frame, parameter_dict, res_dict, db, db_info=None, masks=None, **kwargs):
    ...
    # placeholder for shape if not defined in defo-shape, needed for counting cells
...
def cover_entire_image_with_mask(frame, parameter_dict, res_dict, db, db_info=None, masks=None,
    ...
    # perimeter directly at the edge doesn't work for binary fill holes method later one
...
def deformation(frame, parameter_dict, res_dict, db, db_info=None, masks=None, **kwargs):
    ...
    res_dict[frame]["sum deformations image"].append(["image", np.sum(np.sqrt(u ** 2 + v ** 2)), ""])

    # adding plot of deformation field to the database
    add_plot("deformation", (u, v), show_quiver, frame, db_info, parameter_dict, db)

    # saving raw files
    save_array(db_info["path"], frame + "u", u)
    save_array(db_info["path"], frame + "v", v)
...
def save_array(folder, name, array):
    if default_parameters["output type"] == "txt":
        np.savetxt(os.path.join(folder, name + ".txt"), array)
    elif default_parameters["output type"] == "npy":
        np.save(os.path.join(folder, name + ".npy"), array)
    else:
        raise ValueError("Invalid 'output type'. Use 'txt' or 'npy'")
...
def get_contractillity_contractile_energy(frame, parameter_dict, res_dict, db, db_info=None, masks=None, **kwargs):
    ...
    if isinstance(u, np.ndarray):
        ...
        add_plot("energy_points", [energy_points / ((ps_new * 10 ** -6) ** 2)], show_map_clickpoints, frame, db_info,
                 parameter_dict, db)
    ...
    for obj_id, mask, mtype, warn in mask_iter:
        ...
        if isinstance(u, np.ndarray):
            check_shape(u, t_x)
            contr_energy = np.sum(energy_points[mask_int])  # sum of contractile energy on mask
            # + default_parameters["mask_properties"][mtype]["label"]
...
def traction_force(frame, parameter_dict, res_dict, db, db_info=None, masks=None, **kwargs):
    ...
    if parameter_dict["TFM_mode"] == "finite_thickness":
        tx, ty = TFM_tractions(u, v, pixelsize1=parameter_dict["pixelsize"],
                               ...
                               spatial_filter=parameter_dict["filter_type"], fs=parameter_dict["filter_size"])
    ...
    # saving raw files
    save_array(db_info["path"], frame + "tx", tx)
    save_array(db_info["path"], frame + "ty", ty)
...
def FEM_grid_setup(frame, parameter_dict, mask_grid, db_info=None, warn="", **kwargs):
    ...
    if parameter_dict["FEM_mode"] == "colony":
        ...
        # correcting force for torque and net force
...
def FEM_analysis_borders(frame, res_dict, db, db_info, parameter_dict, stress_tensor, ps_new, borders, obj_id, warn,
    ...
    # currently cells are not detected in cell layer mode
    if parameter_dict["FEM_mode"] == "colony":
...
def FEM_full_analysis(frame, parameter_dict, res_dict, db, db_info=None, masks=None, **kwargs):
    ...
    # add relevant masks, useful in cell layer mode
    ...
    for obj_id, mask_grid, mtype, warn in mask_iter_grid:
        ...
        save_array(db_info["path"], frame + "stress_tensor_xx", stress_tensor[:, :, 0, 0] / (ps_new * 10 ** -6))
        save_array(db_info["path"], frame + "stress_tensor_xy", stress_tensor[:, :, 1, 0] / (ps_new * 10 ** -6))
        save_array(db_info["path"], frame + "stress_tensor_yx", stress_tensor[:, :, 0, 1] / (ps_new * 10 ** -6))
        save_array(db_info["path"], frame + "stress_tensor_yy", stress_tensor[:, :, 1, 1] / (ps_new * 10 ** -6))
...
def apply_to_frames(db, parameter_dict, analysis_function, leave_basics=False, res_dict=None, frames=[], db_info=None,
    ...
    of the gel (or of the cell sheet when applying FEM), height of the gel
...
# code to work on clickpoints outside the addon
if __name__ == "__main__":
    ...
    db_info, masks, res_dict = apply_to_frames(db, parameter_dict, FEM_full_analysis, res_dict=res_dict, frames="01",
                                               db_info=db_info, masks=None)
    """
    ###### problem: produces empty entries when try to access non-existing str
