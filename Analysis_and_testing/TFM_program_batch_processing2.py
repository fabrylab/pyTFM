...
for pos,files in tqdm(files_dict.items(),total=len(files_dict.items())):
    ...
    tx, ty = ffttc_traction(u/pixel_factor, v/pixel_factor, young, pixelsize, bf_image=False, filter="mean")
    ...
    scale_ratio = 0.1   # e.g scale_ratio=0.1: longest arrow will be 10% of image size
    scale = scale_ratio * np.max(np.shape(u_filter)) / np.max(
        np.sqrt((u_filter) ** 2 + (v_filter) ** 2))  # automatic scaleing depending on the image size #
    ...
    scale_ratio = 0.1 # e.g scale_ratio=0.1: longest arrow will be 10% of image size
    scale = scale_ratio * np.max(np.shape(u_no_std)) / np.max(
        np.sqrt((u_no_std) ** 2 + (v_no_std) ** 2))  # automatic scaleing depending on the image size #
    plt.quiver(x1,y1,u_filter*scale,v_filter*scale,scale=1,headwidth=3, scale_units='xy',width=0.002)
    plt.arrow(75, -14, 10*scale, 0, head_width=0, facecolor="black", edgecolor="black",
              clip_on=False, head_starts_at_zero=False, overhang=2)
    plt.text(75, -10, "10 pixel displacement", color="black")
    ...
    scale_ratio = 0.1 # e.g scale_ratio=0.1: longest arrow will be 10% of image size
    scale = scale_ratio * np.max(np.shape(tx)) / np.max(
        np.sqrt((tx / 1000) ** 2 + (ty / 1000) ** 2))  # automatic sacleing in dependace of the image size #
