...
for h_ in [1,20,50,100,200,300]:
    h=h_*pixelsize ## thikness of the gel  (in µm, need same unit as pixelsize)
    ...
    v_expand[max_ind-ax1_length:max_ind, max_ind-ax2_length:max_ind] = v_shift
     ...
     #2) produceing wave vectors (FT-space)
    ...
    kx = np.append(kx1,kx2,axis=1)/(pixelsize*max_ind)  # spatial frequencies: 1/wavelength,in 1/µm in fractions of total length


    ## factor of 2pi?????????!!!!!!!!!??????? ????????!!!!!????? figure this out #### ask ben......
    ...
    c=np.cosh(k*h)
    s=np.sinh(k*h)
    gamma=((3-4*sigma)*(c**2)+(1-2*sigma)**2+(k*h*2*np.pi)**2)/((3-4*sigma)*s*c+k*h*2*np.pi)  ## inf values here because k goes to zero
    ...
    tx_ft = ((-young*ky*c)/(2*k*s*(1+sigma))) * factor1 + ((young*kx)/(2*k*(1-sigma**2))) * gamma * factor2
    tx_ft[0,0]=0 ## zero frequency would represent force every where(constant), so this is legit??
    ty_ft = ((young*kx*c)/(2*k*s*(1+sigma))) * factor1 + ((young*ky)/(2*k*(1-sigma**2))) * gamma * factor2
    ty_ft[0,0]=0
