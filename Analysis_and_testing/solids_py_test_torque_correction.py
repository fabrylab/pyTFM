...
sqr = [(c_x[:-1, :-1], c_y[:-1, :-1]), (c_x[:-1, :-1] + 1, c_y[:-1, :-1]), (c_x[:-1, :-1] + 1, c_y[:-1, :-1] + 1),
       (c_x[:-1, :-1], c_y[:-1, :-1] + 1)]  # needs "counter clockwise orientation
...
loads1 = np.zeros((n, 3))
loads1[:, 0] = np.arange(n)
#loads1[0,2]=-0.001
#loads1[-1,2]= 0.001
for i in range(10, 110, 10):  # forces with non zero moments
    if i <= 50:
        loads1[i - 10:i - 5, 2] = -0.0001
    else:
        loads1[i - 5:i, 2] = 0.0001
...
fx=np.zeros((10,10))
fy=np.zeros((10,10))
fx[nodes[:,2],nodes[:,1]]=loads1[:,1]
fy[nodes[:,2],nodes[:,1]]=loads1[:,2]


com=regionprops((np.zeros((10,10))+1).astype(int))[0].centroid # finding center of mass
...
f[:,:,0]=fx
f[:,:,1]=fy
torques=np.cross(r,f,axisa=2,axisb=2) # note order just changes , this seems to be correct....

np.sum(torques)
q=np.zeros((fx.shape[0],fx.shape[1],2)) # rotated positional vectors
## trying to rotatethe system:
'''
angle=np.pi/8 # in radians


# testing point ortaiton
q[:,:,0] = com[0] + np.cos(angle) * (r[:,:,0] - com[0]) - np.sin(angle) * (r[:,:,1] - com[1])
q[:,:,1] = com[1] + np.sin(angle) * (r[:,:,0] - com[0]) + np.cos(angle) * (r[:,:,1] - com[1])

# this should be related to rotating the individual force angels by angle
q[:,:,0] =  + np.cos(angle) * (f[:,:,0]) - np.sin(angle) * (f[:,:,1]) # whats the mathematics behind this??
q[:,:,1] =  + np.sin(angle) * (f[:,:,0]) + np.cos(angle) * (f[:,:,1])



plt.figure()
plt.quiver(f[:,:,0],f[:,:,1],scale_units="xy")

torques=[]
angles=[]

for angle in np.linspace(-np.pi,np.pi,10000):
    print(angle)
    q[:,:,0] =  + np.cos(angle) * (f[:,:,0]) - np.sin(angle) * (f[:,:,1]) # whats the mathematics behind this??
    q[:,:,1] =  + np.sin(angle) * (f[:,:,0]) + np.cos(angle) * (f[:,:,1])
    torque=np.cross(r,q,axisa=2,axisb=2)
    torques.append(np.sum(torque))
    angles.append(angle)
'''
### go on with this..
# also try finding analytical solution??

# minimization to get angle for torque zero


def get_torque_angle(p):
    q[:, :, 0] = + np.cos(p) * (f[:, :, 0]) - np.sin(p) * (f[:, :, 1])  # whats the mathematics behind this??
    q[:, :, 1] = + np.sin(p) * (f[:, :, 0]) + np.cos(p) * (f[:, :, 1])
    torque=np.abs(np.sum(np.cross(r,q,axisa=2,axisb=2)))
    return torque
...
p = least_squares(fun=get_torque_angle, x0=pstart, method="lm",
                      max_nfev=1000000, xtol=3e-16, ftol=3e-16, args=())["x"]  # trust region algorithm,
...
plt.figure();
plt.quiver(q[:,:,0],q[:,:,1],scale_units="xy")

def construct_loads(nodes,q,n):
    nodes=nodes.astype(int)
    loads = np.zeros((n, 3))
    loads[:, 0] = np.arange(n)
    loads[:,1]=q[nodes[:,1],nodes[:,2],0]
    loads[:, 2] = q[nodes[:, 1], nodes[:, 2], 1]
    return loads
loads=construct_loads(nodes,q,n=100)
get_torque2(nodes,loads)
np.savetxt(os.path.join(folder, "nodes.txt"), nodes, fmt="%i", delimiter="\t")
np.savetxt(os.path.join(folder, "loads.txt"), loads, fmt="%.64f", delimiter="\t")## maybe just dont do that??, 64 is also highest precision
...
t1 = time.time()
UG = sol.static_sol(KG, RHSG, nodes)  # nodes added by me
...
if plot_contours:
    pos.fields_plot(elements, nodes, UC, E_nodes=E_nodes,
                    S_nodes=S_nodes)  # some matplotlib internal function to make it look smoother
end_time = dt.now()
...
# plottingloads
plot_fields(nodes, fields=[loads[:, 1], loads[:, 2]], titles=["x_traction_force", "y_traction_force"], cbar_str="force",
            grid_lines=True)

# plottingloads
plot_fields(nodes, fields=[loads1[:, 1], loads1[:, 2]], titles=["x_traction_force", "y_traction_force"], cbar_str="force",
            grid_lines=True)
...
# plotting straints
plot_fields(nodes, fields=[E_nodes[:, 0], E_nodes[:, 1], E_nodes[:, 2]], titles=["x_strain", "y_strain", "xy_strain"],
            cbar_str="strain")

# plotting stresses
plot_fields(nodes, fields=[S_nodes[:, 0], S_nodes[:, 1], S_nodes[:, 2]], titles=["x_stress", "y_stress", "xy_stress"],
            cbar_str="stress")
