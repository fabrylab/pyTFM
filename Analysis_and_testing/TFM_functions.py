...
def contractility(tx,ty,pixelsize,mask):
    '''
    Calculation of contratile force and force epicenter.Contractile force is the sum of all projection of traction
    ...
    return contractile_force, proj_x, proj_y,center


def contractile_energy(u,v,tx,ty,pixelsize1,pixelsize2,mask):
    pixelsize2*=10**-6 # conversion to µm
    pixelsize1*=10**-6
    energy_points = 0.5 * (pixelsize2 ** 2) * (np.sqrt((tx  * u * pixelsize1) ** 2 + (
            ty  * v * pixelsize1) ** 2))
    bg = np.percentile(energy_points, 50)  # value of a background point
