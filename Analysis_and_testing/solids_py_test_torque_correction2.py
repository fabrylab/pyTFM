...
def correct_torque(fx,fy,mask_area):
    ...
    return q[:, :, 0],q[:, :, 1] # returns corrected forces

def correct_torque2(fx,fy,mask_area):
    com = regionprops(mask_area.astype(int))[0].centroid  # finding center of mass
    com = (com[1], com[0])  # as x y coorinate

    c_x, c_y = np.meshgrid(range(fx.shape[0]), range(fx.shape[1]))  # arrays with all x and y coordinates
    r = np.zeros((fx.shape[0], fx.shape[1], 2))  # array with all positional vectors
    r[:, :, 0] = c_x  # note maybe its also enough to chose any point as refernece point
    r[:, :, 1] = c_y
    r = r - np.array(com)
...
for i in range(10, 110, 10):  # forces with non zero moments
    if i <= 50:
        loads1[i - 10:i - 5, 2] = -0.001
    else:
        loads1[i - 5:i, 2] = 0.001
...
mask_area=np.zeros(fx.shape)+1
mask_area=mask_area.astype(int)
fx_corr,fy_corr=correct_torque(fx,fy,mask_area)



plt.figure();
plt.quiver(fx,fy,scale_units="xy")
plt.quiver(fx_corr,fy_corr,scale_units="xy")

def construct_loads(nodes,fx,fy,n):
    nodes=nodes.astype(int)
    loads = np.zeros((n, 3))
    loads[:, 0] = np.arange(n)
    loads[:,1]=fx[nodes[:,1],nodes[:,2]]
    loads[:, 2] = fy[nodes[:, 1], nodes[:, 2]]
    return loads




#loads=construct_loads(nodes,fx,fy,n=100)


loads=construct_loads(nodes,fx_corr,fy_corr,n=100)
