...
def get_max_values(u_b, v_b, fx_f, fy_f, fx_b, fy_b, stress_tensor_b, exp_test=False, mean_normal_list=None):

    max_dict = defaultdict(lambda: None)
    with suppress(NameError): max_dict["def"] = np.max(np.sqrt(u_b**2+v_b**2))
    with suppress(NameError): max_dict["force"] = \
        np.max(np.concatenate([np.sqrt(fx_f**2+fy_f**2),np.sqrt(fx_b**2+fy_b**2)]))
    with suppress(NameError): max_dict["stress"] = np.max(np.concatenate([(stress_tensor_b[:, :, 0, 0] +
...
def expand_mask(mask,i,method="binary_dilation"):
    ...
    if method == "manual":
        zoom_out = setup_geometry(im_shape=(300, 300), shape_size=i, shape="rectangle")
    return zoom_out
...
if __name__=="__main__":
    ...
    stress_tensors, mean_normal_list, mask_exp_list = load_exp_border(out_folder=out_folder,
                                                                      exp_range=border_ex_test, method=exp_method)
