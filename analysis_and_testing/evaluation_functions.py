...
def standard_measures(mask, pixelsize_tract=1, pixelsize_og=1, mean_normal_list=None, fields=None):
    ...
    u_b, v_b, fx_b, fy_b, fx_f, fy_f, mask_fm, stress_tensor_f, stress_tensor_b = [fields[x] for x in ["u_b", "v_b","fx_b", "fy_b", "fx_f", "fy_f", "mask_fm", "stress_tensor_f", "stress_tensor_b"]]
    mask = mask.astype(bool)
    mask_fm = mask_fm.astype(bool)

    cv_f, cv_b,cont_energy_b, cont_energy_f, contractile_force_b, contractile_force_f,\
       mean_normal_stress_b, mean_normal_stress_f, mean_shear_b, mean_shear_f,avg_normal_stress,rel_av_norm_stress=[None for i in range(12)]
    ...
    with suppress(TypeError, NameError): contractile_force_f, proj_x, proj_y, center = contractillity(fx_f, fy_f, pixelsize_tract, mask_fm)
    ...
    with suppress(TypeError, NameError): contractile_force_b, proj_x, proj_y, center = contractillity(fx_b, fy_b, pixelsize_tract, mask_fm)
    with suppress(TypeError, NameError): cv_f = coefficient_of_variation(binary_erosion(mask), ((stress_tensor_f[:, :, 0, 0] + stress_tensor_f[:, :, 1, 1]) / 2) / (pixelsize_tract), 0)
    with suppress(TypeError, NameError): cv_b = coefficient_of_variation(binary_erosion(mask), ((stress_tensor_b[:, :, 0, 0] + stress_tensor_b[:, :, 1, 1]) / 2) / (pixelsize_tract), 0)
    ...
    measures = {"mask_fm":mask_fm,"mask":mask,"cv_f":cv_f, "cv_b":cv_b, "cont_energy_b":cont_energy_b, "cont_energy_f":cont_energy_f, "contractile_force_b":contractile_force_b,"contractile_force_f":contractile_force_f,
        "mean_normal_stress_b":mean_normal_stress_b, "mean_normal_stress_f":mean_normal_stress_f,"mean_shear_b":mean_shear_b, "mean_shear_f":mean_shear_f, "avg_normal_stress_be":avg_normal_stress_be, "rel_av_norm_stress":rel_av_norm_stress}
    return measures
...
def full_field_comparision(fields):
    fx_b, fy_b, fx_f, fy_f, mask_fm, stress_tensor_f, stress_tensor_b = [fields[x] for x in ["fx_b", "fy_b", "fx_f", "fy_f", "mask_fm", "stress_tensor_f", "stress_tensor_b"]]
    mask_fm = mask_fm.astype(bool)

    res_dict={}
    with suppress(KeyError): res_dict["forces"] = compare_scalar_fields(np.sqrt(fx_b**2+fy_b**2),np.sqrt(fx_f**2+fy_f**2))
    with suppress(KeyError): res_dict["forces_inner"] = compare_scalar_fields(np.sqrt(fx_b[mask_fm] ** 2 + fy_b[mask_fm] ** 2), np.sqrt(fx_f[mask_fm] ** 2 + fy_f[mask_fm] ** 2))
    ...
    with suppress(KeyError): res_dict["shear"] = compare_scalar_fields(stress_tensor_b[:, :, 0, 1], stress_tensor_f[:, :, 0, 1])
    return res_dict
...
def load_exp_border(exp_range=[],out_folder=None, filename_factor=1):
    ...
    mask_exp_list=[]
    for i in tqdm(exp_range):
        stress_tensor_f = np.load(os.path.join(out_folder, "stress_tensors/stress_tensor_f%s.npy"%str(i * filename_factor)))
...
def cut_arrays(fill, arrs, mode="edge"):
    if isinstance(arrs,(list,tuple)):
        ...
        if mode == "match":
            for ar in arrs:
                if isinstance(ar, np.ndarray):
                    e1 = (ar.shape[0] - fill[0]) / 2
                    r1 = int((e1 % 1) * 2)  # potential rest if size diffrence is not divisible by 4
                    e1 = int(np.floor(e1))
                    e2 = (ar.shape[1] - fill[1]) / 2
                    r2 = int((e2 % 1) * 2)  # potential rest if size diffrence is not divisible by 4
                    e2 = int(np.floor(e2))
                    ret.append(ar[e1:-(e1 + r1), e2:-(e2 + r2)])
                else:
                    ret.append(None)
